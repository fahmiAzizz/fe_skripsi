...
export default function SPKMonthlyChart() {
    const [sales, setSales] = useState([]);
    const today = new Date();
    const [month, setMonth] = useState(String(today.getMonth() + 1));
    const [year, setYear] = useState(String(today.getFullYear()));
    const [chartType, setChartType] = useState("bar");
    const [spkType, setSpkType] = useState("best-item"); // New: best, worst, featured

    useEffect(() => {
        const fetchData = async () => {
            await updateDataMonthlySale();
            if (month && year) {
                await fetchSales();
            }
        };
    ...
    const fetchSales = async () => {
        if (!month || !year || !spkType) return;

        try {
            const response = await axios.post(
                `http://localhost:4321/v1/monthlySaleItem/spk/${spkType}?month=${month}&year=${year}`,
                {},
    ...
    const updateDataMonthlySale = async () => {
        try {
            await axios.post(
                `http://localhost:4321/v1/monthlySaleItem`,
                {},
                { withCredentials: true }
            );
        } catch (error) {
    ...
    return (
        <div className="max-w-5xl mx-auto mt-8 bg-white p-6 rounded-lg shadow-lg">
            ...
            <form
                ...
                className="flex flex-wrap items-center gap-4 mb-6 justify-center"
            >
                <select
                    value={month}
            ...
            {/* SPK Table */}
            <h3 className="text-xl font-semibold mt-10 mb-2 text-gray-700 text-center">
